...
var PostgrestFilterBuilder = class extends PostgrestTransformBuilder {
  ...
  */
  in(column, values3) {
    const cleanedValues = Array.from(new Set(values3)).map((s) => {
  ...
  */
  ...
  textSearch(column, query, { config, type: type3 } = {}) {
    let typePart = "";
    if (type3 === "plain") typePart = "pl";
    else if (type3 === "phrase") typePart = "ph";
    else if (type3 === "websearch") typePart = "w";
...
var PostgrestQueryBuilder = class {
  ...
  */
  select(columns, options) {
    const { head: head3 = false, count: count2 } = options !== null && options !== void 0 ? options : {};
  ...
  * ```
  */
  insert(values3, { count: count2, defaultToNull = true } = {}) {
    var _this$fetch;
    const method = "POST";
    const { url, headers } = this.cloneRequestState();
    if (count2) headers.append("Prefer", `count=${count2}`);
    ...
    if (Array.isArray(values3)) {
      const columns = values3.reduce((acc, x) => acc.concat(Object.keys(x)), []);
      if (columns.length > 0) {
  ...
  * ```
  */
  upsert(values3, { onConflict, ignoreDuplicates = false, count: count2, defaultToNull = true } = {}) {
    var _this$fetch2;
    const method = "POST";
    const { url, headers } = this.cloneRequestState();
...
var PostgrestClient = class PostgrestClient2 {
  ...
  * ```
  */
  rpc(fn, args = {}, { head: head3 = false, get: get2 = false, count: count2 } = {}) {
    var _this$fetch;
    let method;
    const url = new URL(`${this.url}/rpc/${fn}`);
    let body;
    const _isObject2 = (v) => v !== null && typeof v === "object" && (!Array.isArray(v) || v.some(_isObject2));
    const _hasObjectArg = head3 && Object.values(args).some(_isObject2);
    if (_hasObjectArg) {
      method = "POST";
      body = args;
    } else if (head3 || get2) {
...
var WebSocketFactory = class {
  ...
  static detectEnvironment() {
    ...
    const gt3 = globalThis;
    if (typeof globalThis !== "undefined" && typeof gt3.WebSocket !== "undefined") {
      return { type: "native", wsConstructor: gt3.WebSocket };
    }
    ...
    if (typeof globalThis !== "undefined" && typeof gt3.WebSocketPair !== "undefined" && typeof globalThis.WebSocket === "undefined") {
      return {
        type: "cloudflare",
    ...
    if (typeof globalThis !== "undefined" && gt3.EdgeRuntime || typeof navigator !== "undefined" && ((_a = navigator.userAgent) === null || _a === void 0 ? void 0 : _a.includes("Vercel-Edge"))) {
      return {
        type: "unsupported",
...
var Serializer = class {
  ...
  _encodeUserBroadcastPush(message, encodingType, encodedPayload) {
    ...
    const view3 = new DataView(header);
    const bytes = new Uint8Array(header);
    let offset = 0;
    view3.setUint8(offset++, this.KINDS.userBroadcastPush);
    view3.setUint8(offset++, joinRef.length);
    view3.setUint8(offset++, ref.length);
    view3.setUint8(offset++, topic.length);
    view3.setUint8(offset++, userEvent.length);
    view3.setUint8(offset++, metadata.length);
    view3.setUint8(offset++, encodingType);
  ...
  _binaryDecode(buffer) {
    ...
    const kind = view3.getUint8(0);
    const decoder = new TextDecoder();
    switch (kind) {
      case this.KINDS.userBroadcast:
        return this._decodeUserBroadcast(buffer, view3, decoder);
    }
  }
  _decodeUserBroadcast(buffer, view3, decoder) {
    const topicSize = view3.getUint8(1);
    const userEventSize = view3.getUint8(2);
    const metadataSize = view3.getUint8(3);
    const payloadEncoding = view3.getUint8(4);
  ...
  _pick(obj, keys4) {
    ...
    return Object.fromEntries(Object.entries(obj).filter(([key]) => keys4.includes(key)));
  }
};
...
var convertCell = (type3, value) => {
  if (type3.charAt(0) === "_") {
    const dataType = type3.slice(1, type3.length);
...
var toArray = (value, type3) => {
  ...
  if (openBrace === "{" && closeBrace === "}") {
    ...
    return arr.map((val) => convertCell(type3, val));
  }
  return value;
};
...
var Channel = class {
  ...
  trigger(event, payload, ref, joinRef) {
    ...
    let eventBindings = this.bindings.filter((bind3) => bind3.event === event && this.filterBindings(bind3, payload, ref));
...
var arrayBufferToBase64 = (buffer) => {
  ...
  for (let i = 0; i < len; i++) {
    binary3 += String.fromCharCode(bytes[i]);
  }
  return btoa(binary3);
};
var LongPoll = class {
...
var serializer_default = {
  ...
  */
  ...
  binaryEncode(message) {
    ...
    let view3 = new DataView(header);
    let offset = 0;
    view3.setUint8(offset++, this.KINDS.push);
    view3.setUint8(offset++, joinRefBytes.byteLength);
    view3.setUint8(offset++, refBytes.byteLength);
    view3.setUint8(offset++, topicBytes.byteLength);
    view3.setUint8(offset++, eventBytes.byteLength);
  ...
  */
  binaryDecode(buffer) {
    ...
    let kind = view3.getUint8(0);
    let decoder = new TextDecoder();
    switch (kind) {
      case this.KINDS.push:
        return this.decodePush(buffer, view3, decoder);
      case this.KINDS.reply:
        return this.decodeReply(buffer, view3, decoder);
      case this.KINDS.broadcast:
        return this.decodeBroadcast(buffer, view3, decoder);
    }
  },
  /** @private */
  decodePush(buffer, view3, decoder) {
    let joinRefSize = view3.getUint8(1);
    let topicSize = view3.getUint8(2);
    let eventSize = view3.getUint8(3);
  ...
  /** @private */
  decodeReply(buffer, view3, decoder) {
    let joinRefSize = view3.getUint8(1);
    let refSize = view3.getUint8(2);
    let topicSize = view3.getUint8(3);
    let eventSize = view3.getUint8(4);
  ...
  /** @private */
  decodeBroadcast(buffer, view3, decoder) {
    let topicSize = view3.getUint8(1);
    let eventSize = view3.getUint8(2);
...
var serialize = (operator, value) => {
  if (operator === "in") {
    ...
    const items = Array.from(new Set(values3)).map((v) => serializeScalar(v)).join(",");
    return `in.(${items})`;
  }
  if (operator === "is") {
...
var RealtimePostgresFilterBuilder = class {
  ...
  add(column, operator, value, negate3 = false) {
    const prefix = negate3 ? "not." : "";
   ...
   */
  in(column, values3) {
    return this.add(column, "in", values3);
  }
...
var RealtimeChannel = class _RealtimeChannel {
   ...
   * ```
   */
  on(type3, filter2, callback) {
    const stateCheck = this.channelAdapter.isJoined() || this.channelAdapter.isJoining();
    const typeCheck = type3 === REALTIME_LISTEN_TYPES.PRESENCE || type3 === REALTIME_LISTEN_TYPES.POSTGRES_CHANGES;
    if (stateCheck && typeCheck) {
      this.socket.log("channel", `cannot add \`${type3}\` callbacks for ${this.topic} after \`subscribe()\`.`);
      throw new Error(`cannot add \`${type3}\` callbacks for ${this.topic} after \`subscribe()\`.`);
    }
    return this._on(type3, filter2, callback);
  }
  /**
  ...
  /** @internal */
  _on(type3, filter2, callback) {
    const typeLower = type3.toLocaleLowerCase();
    const filterValue = filter2 === null || filter2 === void 0 ? void 0 : filter2.filter;
    if (filterValue instanceof RealtimePostgresFilterBuilder || typeof filterValue === "object" && filterValue !== null && typeof filterValue.build === "function") {
      filter2 = Object.assign(Object.assign({}, filter2), { filter: filterValue.build() });
    }
    const ref = this.channelAdapter.on(type3, callback);
    const binding = {
      type: typeLower,
      filter: filter2,
      callback,
  ...
  _updateFilterMessage() {
    this.channelAdapter.updateFilterBindings((binding, payload, ref) => {
      ...
      const bind3 = (_a = this.bindings[typeLower]) === null || _a === void 0 ? void 0 : _a.find((bind4) => bind4.ref === binding.ref);
      if (!bind3) {
        return true;
      }
      if (["broadcast", "presence", "postgres_changes"].includes(typeLower)) {
        if ("id" in bind3) {
          ...
          const bindEvent = (_b = bind3.filter) === null || _b === void 0 ? void 0 : _b.event;
          ...
          const bindEvent = (_f = (_e = bind3 === null || bind3 === void 0 ? void 0 : bind3.filter) === null || _e === void 0 ? void 0 : _e.event) === null || _f === void 0 ? void 0 : _f.toLocaleLowerCase();
...
var handleError = async (error, reject3, options, namespace) => {
  if (error !== null && typeof error === "object" && "json" in error && typeof error.json === "function") {
    ...
    responseError.json().then((err) => {
      ...
      reject3(new StorageApiError(_getErrorMessage(err), status, statusCode, namespace));
    }).catch(() => {
      const statusCode = status + "";
      reject3(new StorageApiError(responseError.statusText || `HTTP ${status} error`, status, statusCode, namespace));
    });
  } else reject3(new StorageUnknownError(_getErrorMessage(error), error, namespace));
};
...
async function _handleRequest(fetcher, method, url, options, parameters, body, namespace) {
  return new Promise((resolve, reject3) => {
    fetcher(url, _getRequestParams(method, options, parameters, body)).then((result) => {
      ...
      return result.json();
    }).then((data) => resolve(data)).catch((error) => handleError(error, reject3, options, namespace));
  });
}
...
var StorageFileApi = class extends BaseApiClient {
  ...
  * - Refer to the [Storage guide](/docs/guides/storage/security/access-control) on how access control works
  */
  async createSignedUrls(paths3, expiresIn, options) {
    var _this9 = this;
  ...
  */
  async remove(paths3) {
    ...
    return _this12.handleOperation(async () => {
      return await remove(_this12.fetch, `${_this12.url}/object/${_this12.bucketId}`, { prefixes: paths3 }, { headers: _this12.headers });
    });
  }
  /**
  ...
  */
  async list(path3, options, parameters) {
    ...
    return _this14.handleOperation(async () => {
      const sortBy3 = (options === null || options === void 0 ? void 0 : options.sortBy) ? _objectSpread22(_objectSpread22({}, DEFAULT_SEARCH_OPTIONS.sortBy), options.sortBy) : DEFAULT_SEARCH_OPTIONS.sortBy;
...
var StorageAnalyticsClient = class extends BaseApiClient {
  ...
  */
  from(bucketName) {
    ...
    return new Proxy(catalog, { get(target, prop3) {
      const value = target[prop3];
...
function retryable(fn, isRetryable) {
  const promise = new Promise((accept, reject3) => {
    ;
    (async () => {
...
function appendFlowIdToRedirectTo(redirectTo, flowId) {
  ...
  if (queryIndex !== -1) {
    ...
    const remaining = base.slice(queryIndex + 1).split("&").filter((pair3) => pair3 !== "" && pair3 !== PKCE_FLOW_ID_PARAM && !pair3.startsWith(`${PKCE_FLOW_ID_PARAM}=`));
...
function userNotAvailableProxy() {
  ...
  return new Proxy(proxyTarget, {
    get: (target, prop3) => {
      if (prop3 === "__isUserNotAvailableProxy") {
        return true;
      }
      if (typeof prop3 === "symbol") {
        const sProp = prop3.toString();
        if (sProp === "Symbol(Symbol.toPrimitive)" || sProp === "Symbol(Symbol.toStringTag)" || sProp === "Symbol(util.inspect.custom)") {
          return void 0;
        }
      }
      throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Accessing the "${prop3}" property of the session object is not supported. Please use getUser() instead.`);
    },
    set: (_target, prop3) => {
      throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Setting the "${prop3}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`);
    },
    deleteProperty: (_target, prop3) => {
      throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Deleting the "${prop3}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`);
    }
  });
}
function insecureUserWarningProxy(user, suppressWarningRef) {
  return new Proxy(user, {
    get: (target, prop3, receiver) => {
      if (prop3 === "__isInsecureUserWarningProxy") {
        return true;
      }
      if (typeof prop3 === "symbol") {
        const sProp = prop3.toString();
      ...
      if (!suppressWarningRef.value && typeof prop3 === "string") {
...
var GoTrueClient = class _GoTrueClient {
  ...
  async signInWithWeb3(credentials) {
    const { chain: chain3 } = credentials;
    switch (chain3) {
      case "ethereum":
      ...
      default:
        throw new Error(`@supabase/auth-js: Unsupported chain "${chain3}"`);
    }
  }
  async signInWithEthereum(credentials) {
    ...
    if ("message" in credentials) {
      ...
      const { chain: chain3, wallet, statement, options } = credentials;
      let resolvedWallet;
      if (!isBrowser()) {
  ...
  async resend(credentials) {
    ...
    try {
      ...
      if ("email" in credentials) {
        const { email, type: type3, options } = credentials;
        let codeChallenge = null;
        let codeChallengeMethod = null;
        if (this.flowType === "pkce") {
          ;
        ...
        const { phone, type: type3, options } = credentials;
  ...
  */
  constructor(supabaseUrl, supabaseKey, options) {
    ...
    if (!settings.accessToken) {
      ...
      this.auth = new Proxy({}, { get: (_, prop3) => {
        throw new Error(`@supabase/supabase-js: Supabase Client is configured with the accessToken option, accessing supabase.auth.${String(prop3)} is not possible`);
      } });
    }
...
var remove2 = _curry3(function remove3(start, count2, list) {
  ...
  switch (path3.length) {
    ...
    default:
      ...
      if (obj[head3] == null) {
        ...
        return assoc_default(head3, dissocPath2(tail2, obj[head3]), obj);
      }
  }
});
...
var export_parse = import_cookie.parse;
var export_serialize = import_cookie.serialize;
export {
  DEFAULT_COOKIE_OPTIONS,
  combineChunks,
  createBrowserClient,
